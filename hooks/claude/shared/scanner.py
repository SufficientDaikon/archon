...
SECRET_PATTERNS = [
    (
        re.compile(
            r'(?:api[_-]?key|apikey)\s*[=:]\s*["\']?' + _SAFE_VALUE + r"[A-Za-z0-9_\-]{20,}", re.I
        ),
        "API key",
    ),
    (
        re.compile(
            r'(?:secret|password|passwd|pwd)\s*[=:]\s*["\']?' + _SAFE_VALUE + r'[^\s"\']{8,}', re.I
        ),
        "Password/secret",
    ),
    (
        re.compile(r'(?:token)\s*[=:]\s*["\']?' + _SAFE_VALUE + r"[A-Za-z0-9_\-\.]{20,}", re.I),
        "Token",
    ),
    (
        re.compile(
...
DANGEROUS_COMMANDS = [
    ...
    (
        re.compile(
            r"\b(DROP|TRUNCATE)\s+(TABLE|DATABASE)\b.*\|\s*(psql|mysql|sqlite3|mongosh)", re.I
        ),
        "destructive SQL piped to database",
    ),
    ...
    (
        re.compile(r"\bgit\s+reset\s+--hard\s+(?:origin/)?(?:main|master)(?![-\w])"),
        "git reset --hard targeting main/master",
    ),
    ...
    (
        re.compile(r"powershell.*Remove-Item.*-Recurse.*-Force", re.I),
        "PowerShell recursive force delete",
    ),
    (re.compile(r"powershell.*Format-Volume", re.I), "PowerShell format volume"),
    (
        re.compile(r"pwsh.*Remove-Item.*-Recurse.*-Force", re.I),
        "PowerShell Core recursive force delete",
    ),
]
