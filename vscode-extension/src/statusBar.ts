...
export class StatusBarManager {
  ...
  constructor() {
    ...
    this._pipelineItem.command = "omniskill.showDashboard";
    this._pipelineItem.tooltip = "OMNISKILL: Click to open Pipeline Dashboard";
    ...
    this._budgetItem.command = "omniskill.showContextBudget";
    this._budgetItem.tooltip = "OMNISKILL: Context budget usage";
    this.setBudget(0, 0);
  ...
  setPipelineIdle(): void {
    this._pipelineItem.text = "$(zap) OMNISKILL: Idle";
    this._pipelineItem.backgroundColor = undefined;
    this._pipelineItem.tooltip =
      "OMNISKILL: No active pipeline — Click to open Dashboard";
  }
  ...
  setPipelineRunning(
    ...
    this._pipelineItem.text = `$(sync~spin) OMNISKILL: Running ${pipelineName} [${currentStep}/${totalSteps}]`;
    this._pipelineItem.backgroundColor = new vscode.ThemeColor(
      "statusBarItem.warningBackground",
    );
    this._pipelineItem.tooltip = `OMNISKILL: Pipeline "${pipelineName}" in progress — Step ${currentStep} of ${totalSteps}`;
  }
  ...
  setPipelineComplete(pipelineName: string): void {
    this._pipelineItem.text = `$(check) OMNISKILL: ${pipelineName} Complete`;
    this._pipelineItem.backgroundColor = undefined;
    this._pipelineItem.tooltip = `OMNISKILL: Pipeline "${pipelineName}" completed successfully`;
  ...
  setPipelineFailed(pipelineName: string, step: string): void {
    this._pipelineItem.text = `$(error) OMNISKILL: ${pipelineName} Failed at ${step}`;
    this._pipelineItem.backgroundColor = new vscode.ThemeColor(
      "statusBarItem.errorBackground",
    );
    this._pipelineItem.tooltip = `OMNISKILL: Pipeline "${pipelineName}" failed at step "${step}" — Click for details`;
  }
  ...
  setBudget(usedTokens: number, totalTokens: number): void {
    if (totalTokens <= 0) {
      ...
      this._budgetItem.tooltip = "OMNISKILL: No context budget data";
      return;
    }
    ...
    this._budgetItem.tooltip = `OMNISKILL: Context budget ${percentage}% used (${usedStr} of ${totalStr})`;
