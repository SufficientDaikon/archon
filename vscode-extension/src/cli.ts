/**
 * Archon CLI Subprocess Wrapper
 *
 * Spawns `archon <command> --json` and parses the JSON envelope response.
 * Handles CLI-not-found gracefully with install prompts.
 * Configurable via `archon.cliPath` setting.
 *
...
function getCliPath(): string {
  ...
  return config.get<string>("cliPath", "archon");
}
...
/**
 * Execute an Archon CLI command and return the parsed JSON response.
 *
  ...
  return new Promise<CliResponse<T>>((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      if (!settled) {
        ...
        reject(
          new Error(
            `Archon CLI timed out after ${timeout / 1000}s running: ${command} ${args.join(" ")}`,
          ),
        );
      }
    }, timeout);
    ...
    proc.on("error", (err: NodeJS.ErrnoException) => {
      ...
      if (err.code === "ENOENT") {
        ...
        reject(new Error(`Archon CLI error: ${err.message}`));
      }
    });
    ...
    proc.on("close", (code) => {
      ...
      if (!jsonText) {
        if (code !== 0) {
          reject(
            new Error(
              `Archon CLI exited with code ${code}: ${stderr.trim() || "No output"}`,
            ),
          );
        } else {
          reject(new Error("Archon CLI returned empty response"));
        }
        return;
      }
      ...
      try {
        ...
        reject(
          new Error(
            `Failed to parse Archon CLI JSON output for '${command}': ${jsonText.substring(0, 200)}`,
          ),
        );
      }
    });
...
/**
 * Check if the Archon CLI is available.
...
export async function showInstallPrompt(): Promise<void> {
  ...
  const choice = await vscode.window.showErrorMessage(
    `Archon CLI not found at "${cliPath}". The extension requires the Archon CLI to function.`,
    "Install with pip",
    "Set CLI Path",
    "Dismiss",
  );
  ...
  switch (choice) {
    case "Install with pip": {
      const terminal = vscode.window.createTerminal("Archon Install");
...
export class CliNotFoundError extends Error {
  constructor(public readonly cliPath: string) {
    super(`Archon CLI not found at: ${cliPath}`);
...
export class CliCommandError extends Error {
  constructor(
    ...
    super(`Archon CLI command '${command}' failed: ${errors.join("; ")}`);
...
/** Initialize Archon in workspace */
export async function initArchon() {
