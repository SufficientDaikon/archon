...
from archon.utils.paths import get_config_path, get_state_path
...
def record_install(
    component_name: str, component_type: str, version: str, platform: str, install_path: str
) -> None:
...
def remove_install_record(component_name: str, platform: str | None = None) -> int:
    ...
    if platform:
        state["installed"] = [
            r
            for r in state.get("installed", [])
            if not (r.get("name") == component_name and r.get("platform") == platform)
        ]
    else:
        state["installed"] = [
            r for r in state.get("installed", []) if r.get("name") != component_name
        ]
...
def get_install_records(
    component_name: str | None = None, platform: str | None = None
) -> list[dict]:
