"""``archon migrate`` — convert legacy skill formats (US-8, FR-045 through FR-048)."""
...
def _migrate_single(source_dir: Path, output_dir: Path) -> dict:
    ...
    # FR-047: Skip if already Archon format
    if fmt == "archon":
        result["status"] = "skipped"
        result["message"] = "Already in Archon format"
        return result
...
def migrate_cmd(
    ...
    """Convert legacy skill formats to Archon format."""
    ...
    if converted:
        console.print("[muted]Next: validate with[/muted] archon validate")
    if failed:
        raise typer.Exit(1)
