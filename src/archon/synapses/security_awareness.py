...
_PATTERNS: list[tuple[re.Pattern, str, str]] = [
    # A1 — Injection
    (
        re.compile(r"\bexec\s*\(|(?<!\w)eval\s*\(", re.I),
        "CRITICAL",
        "A1-Injection: exec/eval (RCE risk)",
    ),
    (
        re.compile(r"subprocess\.(call|run|Popen)\s*\([^,)]*\+", re.I),
        "HIGH",
        "A1-Injection: dynamic subprocess call",
    ),
    (
        re.compile(r"(?:query|execute|cursor\.execute)\s*\([^)]*\+[^)]*\)", re.I),
        "HIGH",
        "A1-Injection: possible SQL concatenation",
    ),
    # A2 — Broken Authentication
    (
        re.compile(
            r"(?:password|passwd|pwd|secret|token|api_?key)\s*[=:]\s*[\"'][^\"']{6,}[\"']", re.I
        ),
        "CRITICAL",
        "A2-Auth: hardcoded credential",
    ),
    (
        re.compile(r"jwt\.decode\s*\([^)]*verify\s*=\s*False", re.I),
        "CRITICAL",
        "A2-Auth: JWT verification disabled",
    ),
    # A3 — XSS
    (
        re.compile(r"\.innerHTML\s*[+]?=|document\.write\s*\(", re.I),
        "HIGH",
        "A3-XSS: raw HTML injection",
    ),
    ...
    (
        ...
        "HIGH",
        "A4-IDOR: dynamic path construction",
    ),
    # A5 — Security Misconfiguration
    (
        re.compile(
            r"ssl\._create_unverified_context|verify\s*=\s*False.*requests\.|VERIFY_SSL\s*=\s*False",
            re.I,
        ),
        "HIGH",
        "A5-Misconfig: SSL verification disabled",
    ),
    (
        re.compile(r"DEBUG\s*=\s*True|debug\s*=\s*True", re.I),
        "MEDIUM",
        "A5-Misconfig: debug mode enabled",
    ),
    ...
    (
        re.compile(
            r"requests\.(get|post|put|delete)\s*\([^)]*user[_-]?input|fetch\s*\([^)]*req\.(body|params|query)",
            re.I,
        ),
        "HIGH",
