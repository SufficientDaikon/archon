...
def validate(context: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(files_changed) > 5:
        ...
        if duplicates:
            issues.append(f"CONSISTENCY: {len(duplicates)} duplicate logic pattern(s) found")
        ...
        if absolute_imports and relative_imports and len(files_changed) > 1:
            issues.append("IMPORTS: Mixed absolute/relative imports across files")
            ...
            for schema in state_schemas[1:]:
                if schema.get("name") == first_schema.get("name"):
                    if schema.get("type") != first_schema.get("type"):
                        issues.append(
                            f"STATE_CONFLICT: {schema.get('name')} has conflicting type definitions"
                        )
