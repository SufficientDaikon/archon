...
def validate(context: dict[str, Any]) -> dict[str, Any]:
    ...
    if pylint_score < min_score:
        issues.append(f"QUALITY: Pylint score {pylint_score:.1f} below minimum {min_score:.1f}")
    ...
    if deprecated_patterns:
        issues.append(f"DEPRECATED: {len(deprecated_patterns)} deprecated pattern(s) found")
