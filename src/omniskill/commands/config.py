"""``omniskill config`` — get/set configuration (US-10, FR-052 through FR-055)."""
...
from omniskill.utils.paths import get_config_path
...
def config_cmd(
    ...
    """Get or set OMNISKILL configuration values."""

    if not is_initialized():
        print_error("OMNISKILL is not initialized. Run [bold]omniskill init[/bold] first.")
        raise typer.Exit(1)
    ...
    if key is None:
        ...
        if is_json():
            ...
            return
        console.print()
        console.rule("[bold cyan]OMNISKILL Configuration[/bold cyan]")
