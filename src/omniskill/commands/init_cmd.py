"""``omniskill init`` — first-time setup (US-1, FR-009 through FR-012)."""
...
from omniskill.core.config import (
...
from omniskill.utils.paths import get_omniskill_home, get_config_path
...
def init_cmd(
    ...
    """Initialize OMNISKILL — detect platforms and create configuration."""

    home_dir = get_omniskill_home()
        ...
        if is_json():
            print_json(json_envelope(
                ...
                errors=[{
                    ...
                    "message": f"OMNISKILL is already initialized at {home_dir}",
                    "detail": str(config_path),
                    "remediation": "Use --force to reinitialize.",
                }],
            ))
            raise typer.Exit(1)
        print_warning(f"OMNISKILL is already initialized at [bold]{home_dir}[/bold]")
    ...
    console.print()
    console.rule("[bold cyan]OMNISKILL Initialization[/bold cyan]")
    console.print()
    ...
    if detected:
        ...
        console.print("  Install a platform, then run [bold]omniskill init[/bold] again,")
        console.print("  or specify one with [bold]omniskill init --platform <name>[/bold].")

    console.print()
    console.print("[muted]Next steps:[/muted]")
    console.print("  omniskill list            — browse available skills")
    console.print("  omniskill install --all   — install everything")
    console.print("  omniskill doctor          — health check")
    console.print()
