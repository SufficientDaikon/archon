...
class TestAntiRationalization:
    ...
    def test_probably_blocked_2(self):
        result = anti_rationalization.validate(
            {"reasoning": "This will probably complete correctly", "task": "build feature"}
        )
        assert result["action"] == "halt"

    def test_probably_blocked(self):
        result = anti_rationalization.validate(
            {"reasoning": "This probably works", "task": "build"}
        )
        assert result["action"] == "halt"

    def test_clean_reasoning(self):
        result = anti_rationalization.validate(
            {"reasoning": "I verified this implementation", "task": "build feature"}
        )
        assert result["action"] == "allow"

    def test_empty(self):
        result = anti_rationalization.validate({"reasoning": "", "task": ""})
        assert result["action"] == "allow"
