...
class TestCodeQualityMetrics:
    ...
    def test_low_pylint_score(self):
        result = code_quality.validate({"code": "x = 1", "pylint_score": 3.0, "min_score": 7.0})
        assert result["action"] == "halt"

    def test_high_complexity(self):
        result = code_quality.validate({"code": "complex code", "cyclomatic_complexity": 15})
        assert result["action"] == "halt"

    def test_missing_type_hints(self):
        result = code_quality.validate({"code": "def func(x): return x", "has_type_hints": False})
        assert result["action"] == "halt"
