#!/usr/bin/env python3
"""
Archon Migration Helper
Converts skills from various formats to Archon format.
...
def create_archon_skill(skill_content: str, metadata: Dict, output_dir: Path) -> bool:
    """
    Creates an Archon-format skill.
...
def migrate_skill(source_path: Path, output_dir: Path, format_hint: Optional[str] = None) -> bool:
    ...
    # Check if already Archon format
    if detected_format == "archon":
        print(f"⚠️  {source_path.name} is already in Archon format")
        return False
    ...
    try:
        ...
        # Create Archon skill
        return create_archon_skill(content, metadata, output_dir)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Migrate skills to Archon format",
