...
def cmd_stats(os_sdk: OmniSkill):
    """Show usage statistics."""
    print_header("OMNISKILL Statistics")
    
    health = os_sdk.health_check()
    
    print(f"{Colors.BOLD}Framework:{Colors.END}")
    print(f"  Version: {health['omniskill_version']}")
...
def cmd_errors(os_sdk: OmniSkill):
...
def cmd_sources(os_sdk: OmniSkill):
...
def cmd_sync(os_sdk: OmniSkill, source_id: str = None, force: bool = False):
...
def cmd_report(os_sdk: OmniSkill, format: str = 'markdown', output: str = None):
    ...
    if format == 'json':
        ...
        report_lines = [
            f"# OMNISKILL Health Report",
            f"",
            f"**Generated**: {timestamp}",
            f"**Version**: {health['omniskill_version']}",
...
def cmd_list(os_sdk: OmniSkill, what: str, tags: str = None, bundle: str = None):
...
def main():
    parser = argparse.ArgumentParser(
        description='OMNISKILL Admin Tool - Framework administration and health checking'
    )
