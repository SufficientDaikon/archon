        ...
        if not manifest_path.exists():
            ...
            if skill_md_path.exists():
                ...
                installed[skill_dir.name] = VersionInfo(
                    skill_dir, {"name": skill_dir.name, "version": "0.0.0"}
                )
            continue
...
def backup_skill(skill_path: Path) -> Path | None:
    """
...
def rollback_skill(skill_name: str, platform: str = "claude-code") -> bool:
    ...
    backups = sorted(
        [b for b in backup_dir.iterdir() if b.name.startswith(skill_name + "_")], reverse=True
    )
...
Examples:
  ...
  python update.py --rollback godot-kit   # Rollback skill
    ...
    parser.add_argument(
        "--apply", nargs="?", const="all", help="Apply updates (all or specific skill)"
    )
    parser.add_argument("--rollback", help="Rollback a skill to previous version")
    parser.add_argument("--platform", default="claude-code", help="Target platform")
