...
## Integration with OMNISKILL

To add async generator pattern to an OMNISKILL skill:

```python
# In skill implementation
from omniskill.tools import ToolStatus

async def my_skill_tool(param: str) -> AsyncGenerator[ToolStatus, None]:
    """OMNISKILL skill with async generator pattern."""
...
**Adopt this pattern for all long-running tools in OMNISKILL to provide excellent real-time UX.**
